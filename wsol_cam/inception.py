...
class InceptionCam(nn.Module):
    ...
    def forward(self, x, labels=None, return_cam=False):
        ...
        x = self.SPG_A3_2b(x)
        x = F.dropout(x, 0.5, self.training) # -> x shape: batch_size x 1024 x 28 x 28
        feat_map = self.SPG_A4(x) # batch_size x num_classes x 28 x 28
        ...
        if return_cam:
        #    feature_map = feat_map.clone().detach()  # batch_size x num_classes x 28 x 28
    ...
    def freeze_layers(self):
        for name, param in self.named_parameters():
            ...
            if self.unfreeze_layer in ['SPG_A4','SPG_A4_2']: 
                types = [self.unfreeze_layer + ".weight", self.unfreeze_layer + ".bias"] # SPG_A4.weight, SPG_A4.bias, SPG_A4_2.weight, SPG_A4_2.bias
                if name in types:
...
def _inception_v3(architecture_type, pretrained=False, pretrained_path=None,**kwargs):
    ...
    if kwargs['ft_ckpt'] is not None:
        print(f'[Debug][inception.py] Loading Fine-tuned Checkpoint: {kwargs["ft_ckpt"]}')
        if unfreeze_layer in ('SPG_A4'):
            print(f'[Debug][inception.py] Load checkpoint to head2!') 
        else: # SPG_A4_2
            print(f'[Debug][inception.py] Load checkpoint to head1!') 
        ...
        for key, value in state_dict.items():
            if key.startswith('module.'): 
                ...
                new_key = key
            if unfreeze_layer in ['SPG_A4']:
                if new_key.startswith('SPG_A4.'):
                    new_key = new_key.replace('SPG_A4.', 'SPG_A4_2.')
...
def inception_v3(architecture_type, pretrained=False, pretrained_path=None,
                 **kwargs):
    return _inception_v3(architecture_type, pretrained, pretrained_path, **kwargs)#model
