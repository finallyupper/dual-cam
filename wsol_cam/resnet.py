...
class ResNetCam(nn.Module):
    ...
    def forward(self, x, labels=None, return_cam=False):
        ...
        if self.last_layer == 'fc':
            logits = self.fc(pre_logit) # batch_size x num_classes
        else:
        ...
        if return_cam:
            ...
            results['cams'] = cams # batch_size x 14 x 14
...
def load_pretrained_model(model, wsol_method, path=None, **kwargs):
    ...
    try:
        ...
        print('[Warning] Strict rule is disabled🚨')
        model.load_state_dict(state_dict, strict=False)
    return model

def _resnet50(architecture_type, pretrained, pretrained_path, **kwargs):
    unfreeze_layer = kwargs['unfreeze_layer']  
    model = {'cam': ResNetCam}[architecture_type](Bottleneck, [3, 4, 6, 3],
                                                  **kwargs)
    if kwargs['ft_ckpt'] is not None:
        print(f'[Debug][resnet.py] Loading Fine-tuned Checkpoint: {kwargs["ft_ckpt"]}')
        if unfreeze_layer in ('fc'):
            print(f'[Debug][resnet.py] Load checkpoint to head2!') 
        else: 
            print(f'[Debug][resnet.py] Load checkpoint to head1!') 
        ...
        if pretrained:
            print(f'[Debug][resnet.py] Loading Pretrained Checkpoint')
