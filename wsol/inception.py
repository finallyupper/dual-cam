...
class Inception3(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.target_layers = [self.Mixed_7c] #CHECK
        ...
        if self.unfreeze_layer != 'all':
            print(f'[Debug][inception.py] Freeze layers: {self.unfreeze_layer}')
            self.freeze_layers()
    ...
    def freeze_layers(self):
        for name, param in self.named_parameters():
            ...
            if self.unfreeze_layer in ['fc', 'fc2']:
                if name.startswith(self.unfreeze_layer):
...
def inception_v3(architecture_type='cam', pretrained=False, pretrained_path=None,
          **kwargs):
    unfreeze_layer = kwargs['unfreeze_layer']
    model = {'cam': Inception3}[architecture_type](**kwargs)

    if kwargs['ft_ckpt'] is not None:
        print(f'[Debug][inception.py] Loading Fine-tuned Checkpoint: {kwargs["ft_ckpt"]}')
        if unfreeze_layer in ('fc'):
        ...
        if pretrained:
            ...
            if pretrained_path is None:
                state_dict = load_url(model_urls['inception_v3_google'], progress=True) 
            else:
                state_dict = torch.load(os.path.join(pretrained_path, 'inceptionv3.pth'))
            ...
            state_dict = remove_layer(state_dict, 'AuxLogits') 
