...
def vit(architecture_type='cam', pretrained=False, **kwargs):
    ...
    if kwargs['ft_ckpt'] is not None and kwargs['dataset_name'] != 'ILSVRC':
        ...
        print(f'[Debug] Load checkpoint from {kwargs["ft_ckpt"]}')
        if unfreeze_layer in ('head2'):
            print(f'[Debug][vit.py] Load checkpoint to head2!') 
        else: print(f'[Debug][vit.py] Load checkpoint to head1!')

        state_dict = torch.load(kwargs['ft_ckpt'])['state_dict']
        new_state_dict = {}
        for key, value in state_dict.items(): # ex key : module.features.0.weight
